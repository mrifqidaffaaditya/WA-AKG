...
export async function PUT(
    ...
    try {
        ...
        const { sessionId, action, participants } = body;

        if (!sessionId || !action || !participants || !Array.isArray(participants)) {
            return NextResponse.json({ 
                error: "sessionId, action, and participants (array) are required" 
            }, { status: 400 });
        }
